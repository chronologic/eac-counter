...
class EacCounter {
  constructor({ nomicsApiKey, etherscanApiKey } = {}) {
  ...
  async enableUSDFetching(apiKey) {
    ...
    const url = `https://api.nomics.com/v1/exchange-rates/history?key=${
      apiKey || this._nomicsApiKey
    }&currency=ETH&start=${startDate.toISOString()}`;

    let ethToUsdAtTimestampValues;

    try {
      ethToUsdAtTimestampValues = await fetch(url).then(
  ...
  getUsdValueAtTime(weiAmount, timestamp) {
    ...
    const timePeriod = this.ethToUsdAtTimestampValues.find(
      (timePeriod) =>
        timePeriod.timestamp === roundTxDate.format('YYYY-MM-DD[T]HH:mm:ss[Z]')
    );
  ...
  async getTotalTransferred() {
    ...
    const baseUrl = `https://api.etherscan.io/api?module=account&action=txlist&startblock=0&endblock=99999999&sort=asc&apiKey=${this._etherscanApiKey}`;
      ...
      const resultPromise = fetch(url).then(async (resp) => {
        ...
        if (response.status == '1' && response.message === 'OK') {
          const weiTransferred = response.result.reduce(
            (acc, tx) => acc + parseInt(tx.value),
            0
          );
          const usdTransferred = this._canFetchUSD
            ? response.result.reduce(
                (acc, tx) =>
                  acc + this.getUsdValueAtTime(tx.value, tx.timeStamp),
                0
              )
            : null;
    ...
    try {
      ...
      totalEthTransferred = values.reduce(
        (acc, value) => acc + parseFloat(value.eth),
        0
      );
      if (this._canFetchUSD) {
        totalUsdTransferred = values.reduce(
          (acc, value) => acc + parseFloat(value.usd),
          0
        );
      }
    } catch (e) {
